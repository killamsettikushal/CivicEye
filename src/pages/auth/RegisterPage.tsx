...
import { Eye, Lock, User, ArrowRight, AlertCircle, KeyRound } from 'lucide-react';
import { useAuth } from '@/contexts/AuthContext';
import { useToast } from '@/contexts/ToastContext';
import { validateEmail, validatePassword, PASSWORD_RULES } from '@/services/authService';
...
export function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      navigate('/registration-success', { state: { username: result.username } });
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950 py-8">
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Join as a citizen to report issues and earn rewards</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            {errors.fullName && <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{errors.fullName.message}</p>}
          </div>
          ...
          <div>
            ...
            {errors.password && <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{errors.password.message as string}</p>}
            <div className="mt-1.5 text-xs text-slate-400">
              Min {PASSWORD_RULES.minLength} chars with uppercase, lowercase, digit & special character
            </div>
          </div>
